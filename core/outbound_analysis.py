...
class OutboundAnalyzer:
    ...
    def aggregate_daily_data(self, df: pd.DataFrame, date_column: str, 
        ...
        try:
            ...
            if order_column and order_column in df.columns:
                agg_dict[f'日出订单数'] = (order_column, 'nunique')
                
            # 如果有SKU列，计算每日SKU数（去重）
            if sku_column and sku_column in df.columns:
                agg_dict[f'日出SKU数'] = (sku_column, 'nunique')
                
            # 如果有数量列，计算每日件数总和
            if quantity_column and quantity_column in df.columns:
                agg_dict[f'日出件数'] = (quantity_column, 'sum')
            ...
            # 处理列名（确保正确的列名映射）
            if len(daily_data.columns) > len(agg_dict) + 1:  # 如果有多级索引
                # flatten多级索引
                daily_data.columns = [daily_data.columns[0]] + list(agg_dict.keys())
            
            # 按日期排序
            daily_data = daily_data.sort_values(date_column)
            
            st.success(f"✅ 数据聚合完成：共 {len(daily_data)} 天的出库数据")
    ...
    def aggregate_daily_data_enhanced(self, df: pd.DataFrame, date_column: str,
                                    ...
                                    order_count_column: Optional[str] = None,
                                    sku_column: Optional[str] = None,
                                    sku_count_column: Optional[str] = None,
                                    item_column: Optional[str] = None,
                                    item_count_column: Optional[str] = None) -> pd.DataFrame:
        """
        增强版按日期聚合出库数据 - 支持原始数据和聚合数据
        
        Args:
            df: 数据框
            date_column: 日期列名
            order_id_column: 订单号列名（原始数据，需要聚合）
            order_count_column: 订单数/天列名（已聚合数据）
            sku_column: SKU列名（原始数据，需要聚合）
            sku_count_column: SKU数/天列名（已聚合数据）
            item_column: 件数列名（原始数据，需要聚合）
            item_count_column: 件数/天列名（已聚合数据）
        ...
        try:
            ...
            if order_id_column and order_id_column in df.columns:
                # 原始订单号数据：按日期聚合计算订单数/天
                agg_dict['订单数/天'] = (order_id_column, 'nunique')
                st.info(f"📦 将按日期聚合订单号 '{order_id_column}' 计算订单数/天")
            elif order_count_column and order_count_column in df.columns:
                # 已聚合的订单数/天数据：直接使用
                agg_dict['订单数/天'] = (order_count_column, 'sum')
                st.info(f"📦 直接使用已聚合的订单数/天数据 '{order_count_column}'")
            
            # 处理SKU数据
            if sku_column and sku_column in df.columns:
                # 原始SKU数据：按日期聚合计算SKU数/天
                agg_dict['SKU数/天'] = (sku_column, 'nunique')
                st.info(f"🏷️ 将按日期聚合SKU '{sku_column}' 计算SKU数/天")
            elif sku_count_column and sku_count_column in df.columns:
                # 已聚合的SKU数/天数据：直接使用
                agg_dict['SKU数/天'] = (sku_count_column, 'sum')
                st.info(f"🏷️ 直接使用已聚合的SKU数/天数据 '{sku_count_column}'")
            
            # 处理件数数据
            if item_column and item_column in df.columns:
                # 原始件数数据：按日期聚合计算件数/天
                agg_dict['件数/天'] = (item_column, 'sum')
                st.info(f"🔢 将按日期聚合件数 '{item_column}' 计算件数/天")
            elif item_count_column and item_count_column in df.columns:
                # 已聚合的件数/天数据：直接使用
                agg_dict['件数/天'] = (item_count_column, 'sum')
                st.info(f"🔢 直接使用已聚合的件数/天数据 '{item_count_column}'")
    ...
    def analyze_batch_enhanced(self, df: pd.DataFrame, date_column: str,
                             ...
                             sku_count_column: Optional[str] = None,
                             item_column: Optional[str] = None,
                             item_count_column: Optional[str] = None,
                             start_date: Optional[datetime] = None,
                             end_date: Optional[datetime] = None) -> Tuple[pd.DataFrame, Dict]:
        """
        增强版批量分析出库数据 - 支持原始数据和聚合数据
        
        Args:
            df: 原始数据框
            date_column: 日期列名
            order_id_column: 订单号列名（原始数据）
            order_count_column: 订单数/天列名（聚合数据）
            sku_column: SKU列名（原始数据）
            sku_count_column: SKU数/天列名（聚合数据）
            item_column: 件数列名（原始数据）
            item_count_column: 件数/天列名（聚合数据）
            start_date: 开始日期
            end_date: 结束日期
        ...
        try:
            ...
            # 3. 智能数据处理：区分原始数据和聚合数据
            daily_data = self.aggregate_daily_data_enhanced(
                df_cleaned, date_column, 
                order_id_column, order_count_column,
                sku_column, sku_count_column,
                item_column, item_count_column
            )
    ...
    def analyze_batch(self, df: pd.DataFrame, date_column: str,
        ...
        try:
            ...
            daily_data = self.aggregate_daily_data(
                df_cleaned, date_column, order_column, sku_column, quantity_column
            )
    ...
    def filter_outliers_98_percentile(self, data: pd.Series) -> pd.Series:
        ...
        try:
            ...
            # 在streamlit中显示而不是print
            # st.write(f"数据分析 - 中位数: {median:.1f}, Q1: {q1:.1f}, Q3: {q3:.1f}, 均值: {mean_val:.1f}")
            
            # 🔍 第二步：识别异常低值（业务逻辑判断）
            # 对于订单数、SKU数、件数等业务指标，小于10的值通常是异常的
            if median > 50:  # 正常业务数据的中位数应该比较大
                min_business_value = max(10, q1 * 0.1)  # 业务最小值不应小于Q1的10%，且不小于10
                # st.write(f"业务最小值阈值: {min_business_value:.1f}")
            else:
                min_business_value = max(1, median * 0.1)  # 对于较小的数据，允许更小的值
                # st.write(f"业务最小值阈值（小数据）: {min_business_value:.1f}")
            ...
            if len(filtered_data) < len(data) * 0.3:  # 如果过滤掉超过70%的数据
                # st.write("过滤过于严格，使用备用方案")
                # 使用更宽松的5%分位数作为下边界
                lower_backup = data.quantile(0.05)
                filtered_data = data[data >= lower_backup]
            
            # st.write(f"单向过滤结果 - 原始: {len(data)}个, 过滤后: {len(filtered_data)}个, 移除低值: {len(data) - len(filtered_data)}个")
            # st.write(f"过滤后范围: {filtered_data.min():.1f} ~ {filtered_data.max():.1f} (保留所有高值)")
            ...
            for col, stats in summary.items():
                if isinstance(stats, dict) and 'daily_avg_no_outliers' in stats:
                    ...
                    if stats.get('trend') == 'increasing':
                        suggestions.append(f"📈 {col}呈上升趋势，建议关注产能规划")
                    elif daily_max > avg_no_outliers * 2:
                        suggestions.append(f"⚠️ {col}波动较大，建议分析峰值日期的特殊原因")
            
            # 基于数据分布的建议
            if len(daily_data) >= 7:
                suggestions.append("📅 建议按周/月维度进一步分析数据规律")
            
            if len([s for s in suggestions if not s.startswith("📊") and not s.startswith("📅")]) == 0:
                suggestions.append("💡 数据表现平稳，建议持续监控关键指标")
