...
from analysis_engine import AnalysisEngine, DimensionConfigManager
...
def render_sidebar():
    ...
    if uploaded_file:
        # 显示文件信息
        file_size = FileUtils.format_file_size(uploaded_file.size)
...
def execute_analysis():
    ...
    try:
        ...
        if preprocessing_steps:
            st.write("## 🧹 前置数据处理")
            success = analysis_engine.execute_preprocessing(preprocessing_steps)
            if not success:
                st.error("❌ 前置处理失败")
                return
